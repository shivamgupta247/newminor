...
import { getNeetQuestionsBySubject, getNeetQuestionsByTopic, neetQuestionBank, neetSubjects } from "@/data/neetQuestionBank";
...
const NeetQuiz = () => {
    ...
    if (quizState?.isCompleted && quizState.questions?.length) {
      (async () => {
        ...
        if (quizState.mode === 'calibration') {
          ...
          setRating('neet', initialMap[cat], subject);
          if (user?.id) await saveQuizCategory(user.id, 'neet', cat);
        } else {
          const delta = ratingChangeFromPerformance((perf.correct / perf.total) * 100);
          const currentRating = getRating('neet', subject);
          setRating('neet', currentRating + delta, subject);
          const cat = getCategory('neet', subject);
          if (user?.id) await saveQuizCategory(user.id, 'neet', cat);
        }
        ...
        category: getCategory('neet', subject),
        rating: getRating('neet', subject),
        timeTaken: totalTime,
  ...
  const handleStart = (quizType: 'topic' | 'subject' | 'full', duration: number, subject?: string, topic?: string) => {
    ...
    if (quizType === 'full') {
      ...
      questionPool = neetQuestionBank;
      targetCount = 15;
      category = getCategory('neet'); // Use default category
    } else if (quizType === 'topic' && topic && subject) {
      questionPool = getNeetQuestionsByTopic(subject, topic);
      ...
      questionPool = getNeetQuestionsBySubject(subject);
      targetCount = 12;
      category = getCategory('neet', subject);
    } else {
  ...
  const handleCalibrationStart = (duration: number, subject: string) => {
    const questionPool = getNeetQuestionsBySubject(subject);
  ...
  if (stage === 'start') {
    return <QuizStart onStart={handleStart} examName="NEET" subjects={neetSubjects} getTopicsBySubject={(subjectId) => {
      const subject = neetSubjects.find(s => s.id === subjectId);
